...
@section('content')
<div class="mt-12">
    <h1 class="text-3xl text-blue-800 mb-6 text-center">
        Creez un compte pour ajouter une annonce
    </h1>
    <form method="POST" action="{{ route('register') }}" class="w-full max-w-sm mx-auto rounded-lg border shadow-md p-5 mb-5">
        ...
        <div class="mb-4">
            ...
            <input id="name" type="text" name="name" class="shadow border rounded w-full p-2" value="{{ old('name') }}" autocomplete="name" placeholder="Nom de boutique / restaurant ..." autofocus>
        @error('name')
        ...
        <p for="role-select" class="font-semibold text-gray-700">
            Je suis un :
        </p>
        <div class="flex justify-between items-center">
            <label for="client">
                Magasin en tout genre
                <input type="radio" value="1" id="client" name="role_id">
        ...
        <button type="submit" class="bg-blue-800 text-white hover:bg-blue-900 transition ease-in-out duration-500 rounded-md shadow-md w-full block px-4 py-2 mt-3">
            Créer mon compte
        </button>
    </form>
</div>
